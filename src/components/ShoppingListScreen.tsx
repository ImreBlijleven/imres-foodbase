...
export function ShoppingListScreen({ weekPlan, onBack }: Props) {
  const { shoppingList, generateFromWeekPlan, toggleItem, addManualItem, removeItem } =
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {!supermarktMode && (
        <form onSubmit={handleAddItem} className="p-4 border-t border-gray-100 bg-white flex gap-2">
          <input
